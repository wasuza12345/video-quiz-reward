...
export function usePlayerProgress({ player, active, fallbackPositionSec, fallbackFurthestSec, getMaxReached }: UsePlayerProgressOptions): PlayerProgress {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      if (now - lastTickAt >= TICK_INTERVAL_MS) {
        lastTickAt = now;
        setLive({ positionSec: player.getCurrentTime(), furthestSec: getMaxReached() });
      }
      rafId = requestAnimationFrame(loop);
    };
    rafId = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(rafId);
  }, [player, active, getMaxReached]);
  ...
  // and this is at most one frame stale in the latter.
  return { positionSec: player.getCurrentTime(), furthestSec: getMaxReached() };
}
