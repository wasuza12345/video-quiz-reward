...
export interface SessionWriterApi {
  ...
  sendImmediate: (type: Exclude<ClientEventType, "TICK">, positionSec: number, clientAt?: string) => Promise<EventsApplyResponse | null>;
...
/** Wraps SessionWriter for React: creates one per sessionId, dispatches SEQ_CONFLICT universally. */
export function useSessionWriter(sessionId: string | null, dispatch: React.Dispatch<WatchAction>): SessionWriterApi {
  const writer = useMemo(() => (sessionId ? new SessionWriter(makePost(sessionId)) : null), [sessionId]);
  ...
  const sendImmediate = useCallback<SessionWriterApi["sendImmediate"]>(
    async (type, positionSec, clientAt) => {
      if (!writer) return null;
      return interpret(await writer.sendImmediate(type, positionSec, clientAt));
    },
    [writer, interpret],
  );
